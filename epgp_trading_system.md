...
**Verification that B = 1:** For each PDE, the Noetherian multiplier B satisfies A(z) · B(x, z) = 0 for all z ∈ V (where A(z) is the principal symbol, a scalar for these single-equation cases). Since A(z) = 0 by definition on V, the trivial choice B(x, z) = 1 satisfies this condition for any smooth variety point. The variety is smooth for all four PDEs above (the symbol p(z) has non-vanishing gradient ∇p ≠ 0 everywhere on V — verified by inspection: ∇p = (1, -2z₂) for the 1D heat equation, never zero). Therefore no higher-order Noetherian multipliers are needed, and s = 1, B = 1 for all cases.
...
**Implementation note:** The closed-form Q above must be verified against the numerical quadrature fallback (Section 3.6.6) before use. Compute relative error |k_closed - k_quad| / |k_quad| on a grid covering σ ∈ [0.1, 2.0], t, t' ∈ [0, 0.5], x, x' ∈ [-3, 3]. If max relative error exceeds 10⁻⁶, use the quadrature fallback as the default implementation and treat the closed form as informational only.
...
**Performance:** Quadrature order N = 20–30 per dimension gives relative error < 10⁻⁶ for typical parameter ranges (σ ∈ [0.1, 10], τ, τ' ∈ [0, 0.5], x, x' ∈ [-5, 5]). This tolerance is sufficient for trading applications (confidence scores are rounded to 2 decimal places). Increase to N = 50 if extreme parameters are encountered (σ > 5, τ > 1). For m = 100 observation points, the full Gram matrix requires m(m+1)/2 × N² ≈ 4.5 × 10⁶ exponential evaluations — sub-second on modern hardware. Vectorization with NumPy broadcasting can reduce this by 10–100×.
...
**Implementation details for heat equation:**

- **Parametrization:** Represent each z₂ as a pair (a, b) ∈ ℝ² with z₂ = a + bi, z₁ = z₂² = (a²-b²) + 2abi. Optimize over (a, b) directly — this avoids complex arithmetic in the optimizer.
- **Gradient via chain rule:** ∂K/∂a = Re and Im parts of 2z₂ · ∂K/∂z₁. In PyTorch, use `torch.view_as_real()` to separate complex tensors into (real, imag) components and apply `.requires_grad_()` on the real representation.
- **Initialization:** Compute the data extent [x_min, x_max] and [τ_min, τ_max] from the observation set. For the heat variety z₁ = z₂², the relevant frequencies satisfy |z₂| ~ 1 / data_scale. Initialize r spectral points uniformly in the box a ∈ [-2/Δx, 2/Δx], b ∈ [-2/Δτ, 2/Δτ] where Δx = x_max - x_min and Δτ = τ_max - τ_min.
...
Compute log V first, then exponentiate only at the final step. For variance propagation, apply the delta method: Var[V] ≈ (∂V/∂u)² · Var[u] = prefactor² · Var[u]. In log-space, recover u = exp(log_u) and Var[u] = u² · exp(log_var_u) (delta method on log scale), then compute Var[V] = prefactor² · Var[u]. This avoids intermediate overflow/underflow entirely.
...
def inverse_transform_logspace(x, tau, log_u, log_var_u, K, alpha, beta):
    ...
    u = np.exp(log_u)
    var_u = u**2 * np.exp(log_var_u)  # delta method: Var[u] = u² · Var[log u]
    var_V = prefactor**2 * var_u
    return np.exp(log_V), var_V
```
...
- **Option 1 (recommended for initial implementation):** Exclude near-expiry contracts. Formally: τ_min = 3/252 years (3 trading days). Contracts with T - t < τ_min are excluded from the EPGP fit. If fewer than 10 contracts remain after applying all filters (including this cutoff), skip EPGP inference for that date entirely and carry forward the previous day's confidence score with a staleness flag. No smooth taper is applied; the hard cutoff is sufficient for initial deployment.
...
2. Filter: bid > 0, ask > 0, volume > 0, 0.01 ≤ |delta| ≤ 0.50 (exclude deep ITM and near-zero delta). Delta is computed using Black-Scholes closed-form with mid implied vol (see step 4).
3. Compute mid price: V_mid = (bid + ask) / 2. No volume weighting — simple midpoint is used to avoid look-ahead dependency on intraday volume. If bid-ask spread > 20% of mid, flag as wide-spread and optionally exclude.
4. Extract implied vol: compute IV from V_mid using Brent's method on the Black-Scholes formula. Use ATM IV (the IV of the option with |delta| closest to 0.50) as σ for the coordinate transform. If ATM IV is unavailable, fall back to VIX/100 as proxy and log a warning.
5. For each (K, T) pair, transform to heat coordinates: x = ln(S/K), τ = ½σ²(T - t).
   - Compute u = V_mid / (K · exp(-αx - βτ)).
6. Result: observation set {(xᵢ, τⱼ, uᵢⱼ)} ready for EPGP.
